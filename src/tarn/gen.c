...
asmop *const get_register_asmop(const char *name) {
...
tarn_init_asmops (void)
{
  ...
  /* asmop_zero.type = AOP_REG; */
  /* asmop_zero.size = 1; */
  /* asmop_adl.aopu.bytes[0].in_reg = true; */
  /* asmop_adl.aopu.bytes[0].byteu.reg = tarn_regs + ZERO_IDX; */
...
////////////////////////////////////////////////////////////////////////////////
void aop_cmp(unsigned int op, asmop *left, asmop *right, asmop *result, symbol *true_branch, symbol *false_branch, bool invert);
void aop_move_byte(asmop *a1, asmop *a2, int index);
asmop *aop_byte_of(asmop *aop, unsigned index);
...
asmop *aop_byte_of(asmop *aop, unsigned index) {
#define ABO_DEBUG { emit2("", "; aop_byte_of failure (%s:%d)", __FILE__, __LINE__); emit_asmop("aop", aop); }
    // We reverse immd_off for direct and immediate AOPs in this
    // function because addresses are stored big-endian. (Who decided
    // that?)

    if (index >= aop->size) {
        wassertl (index < aop->size, "aop_byte_of: index >= aop->size");
    }

    if (AOP_IS_DIRECT(aop)) {
        asmop *newop = newAsmop (AOP_IMMD);
        newop->size = 1;
        newop->aopu.immd = aop->aopu.aop_dir;
        newop->aopu.immd_off = aop->size - index - 1;
        return newop;
    }

    if (AOP_IS_REG(aop)) {
        asmop *newop = get_register_asmop(aop->aopu.bytes[index].byteu.reg->name);
        if (!newop) {
            ABO_DEBUG;
            return ASMOP_STACK;
        }
    ...
    if (AOP_IS_LIT(aop)) {
        ...
        asmop *newop = newAsmop (AOP_LIT);
        newop->size = 1;
        newop->aopu.aop_lit = constCharVal(value);
        return newop;
    }
    ...
    if (a2->size == 1) {
        ...
        if (AOP_IS_REG(a2) || AOP_IS_LIT(a2)) {
            ...
            for (int i = 0; i < a2->size; ++i) {
                aop_move_byte(ASMOP_STACK, a2, i);
...
static bool aop_move_direct(asmop *a1, asmop *a2) {
    if (a1->size == a2->size) {
        ...
        for (int i = 0; i < a1->size; ++i) {
            aop_move(aop_byte_of(a1, i), aop_byte_of(a2, i));
        }
    }
    ...
    if (a1->size == a2->size) {
        if (AOP_IS_DIRECT(a2) || AOP_IS_CODE(a2)) {
            if (a1 == ASMOP_ADHL) {
                ...
                for (int i = 0; i < a2->size; ++i) {
                    // backwards because ADHL is backwards...
                    emit_mov(a1->aopu.bytes[i].byteu.reg->name, "stack");
                }
            } else {
                for (int i = 0; i < a2->size; ++i) {
                    load_address_16o(a2->aopu.aop_dir, i);
            ...
            if (a1 == ASMOP_MEM && a2 == ASMOP_MEM) {
                emit2(";", "You can't move mem to itself!");
                AOP_MOVE_DEBUG;
                return false;
            }
            ...
            if (a1 == ASMOP_STACK) {
                ...
                for (int i = 0; i < a1->size; ++i) {
                    int val = byteOfVal(a2->aopu.aop_lit, i);
                    if (val) {
                        emit_mov_lit(a1->aopu.bytes[i].byteu.reg->name, val);
                    } else {
                        emit_mov(a1->aopu.bytes[i].byteu.reg->name, "zero");
                    }
                }
            }
        } else if (AOP_IS_SPILL(a2)) {
            ...
            // SFRs are always size 1, so this works
        ...
        if (a1 == ASMOP_STACK) {
            for (int i = 0; i < a2->size; ++i) {
                aop_move(ASMOP_STACK, aop_byte_of(a2, i));
            }
        } else {
            for (int i = 0; i < a1->size; ++i) {
                aop_move(aop_byte_of(a1, i), aop_byte_of(a2, i));
...
static bool aop_move_immediate(asmop *a1, asmop *a2) {
    if (a1->size == a2->size) {
        if (AOP_IS_IMMEDIATE(a2)) {
            if (a1->size < 2) {
                ...
                AOP_MOVE_DEBUG;
                /* for (int i = 0; i < a2->size; ++i) { */
                /*     load_address_16o(a1->aopu.immd, i + a1->aopu.immd_off); */
                /*     aop_move(ASMOP_STACK, aop_byte_of(a2, i)); */
                /*     load_address_16o(a2->aopu.immd, i + a2->aopu.immd_off); */
...
static bool aop_move(asmop *a1, asmop *a2) {
    ...
    if (AOP_IS_IMMEDIATE(a1)) {
        return aop_move_immediate(a1, a2);
    }
    ...
    if (AOP_IS_DIRECT(a1)) {
        ...
        if (a1->size == 1) {
            if (AOP_IS_LIT(a2)) {
                ...
                if (index == 0) {
                    emit2("mov", "%s il ,lo8(%s + %d)", a1->aopu.aop_dir, a2->aopu.immd, a2->aopu.immd_off);
                    cost(1);
                } else if (index == 1) {
                    emit2("mov", "%s il ,hi8(%s + %d)", a1->aopu.aop_dir, a2->aopu.immd, a2->aopu.immd_off);
                    cost(1);
                } else {
                    AOP_MOVE_DEBUG;
                }
                ...
                TRACE;
                load_address_16o(a2->aopu.immd, a2->aopu.immd_off + (a2->size - index - 1));
                aop_move(a1, ASMOP_MEM);
            } else {
                AOP_MOVE_DEBUG;
            }
        ...
        if (a1->size == 1) {
            if (AOP_IS_DIRECT(a2) || AOP_IS_CODE(a2)) {
                if (a2->size == 2) {
                    // Offset is reverse of index, because index=0 is LSB but offset=0 is MSB.
                    load_address_16o(a2->aopu.aop_dir, a2->size - index - 1);
                    aop_move(a1, ASMOP_MEM);
                } else if (a2->size == 1) {
                    load_address_16(a2->aopu.aop_dir);
                    // aop->aopu.bytes[i].byteu.reg = sym->regs[i];
                ...
                emit2("mov", "%s il ,%s + %d", a1->aopu.bytes[0].byteu.reg->name, a2->aopu.immd, a2->aopu.immd_off + index);
                cost(1);
                /* emit2(";", "implement reg <- immediate"); */
...
static void genGetByte(iCode *ic) {
    if (!regalloc_dry_run) { DEBUG_GEN_FUNC("genGetByte      = ", ic); }
    ...
    if (IS_OP_LITERAL(right)) {
        ...
        if (size_result == 1) {
                        ...
                        if (!val) {
                            if (OP_SYMBOL(left)->isspilt) {
                                ...
                                if (is_mem(result)) {
                                    ...
                                    emit2("", "; working on (%s:%d)", __FILE__, __LINE__);
                                    aopOp(result);
                                    aopOp(left);
                                    aopOp(right);
                                    emit_asmop("left  ", left->aop);
                                    ...
                                    aop_alu(ALUS_PLUS, left->aop, right->aop, ASMOP_STACK, NULL);
                                    emit2("", "; does it work?");
                                }
                            } else {
                                emit2("", "; implement me (%s:%d)", __FILE__, __LINE__);
            ...
            /* for (int i = 0; i < result->aop->size; ++i) { */
                aop_move(result->aop, ASMOP_STACK);
            /* } */
            emit2("", "; does it work?");
        }
    } else {
        ...
        if (operandSize(left) == 1) {
            ...
            if (IS_SYMOP(left)) {
                if (OP_SYMBOL(left)->remat) {
                    ...
                    emit2("mov", "stack mem", op_get_mem_label(left));
    ...
    if (size_result == 2 && size_right == 1) {
        ...
        if (is_mem(right)) {
            ...
            if (is_mem(result)) {
                ...
                emit_asmop("result", result->aop);
                emit_asmop("right", right->aop);
                aop_move_byte(result->aop, right->aop, 0);
            } else if (is_reg(result)) {
                const char *result_reg = op_get_register_name(result);
                const char *right_reg = op_get_register_name(right);
                if (result_reg && right_reg && strcmp(result_reg, right_reg)) {
        ...
        aopOp(right);
        aop_move(result->aop, right->aop);
    } else if (size_result == size_right) {
        aopOp(result);
        aopOp(right);
        aop_move(result->aop, right->aop);
    } else {
    ...
    if (IS_SYMOP (cond)) {
        if (OP_SYMBOL(cond)->regType == REG_CND)  {
            ...
            if (cond->aop->size == 1) {
                ...
                aop_cmp(ic->op, cond->aop, ASMOP_ZERO, NULL, IC_TRUE(ic), IC_FALSE(ic), false);
                do_gen_ifx = false;
            } else {
                emit2(";", "implement me");
            }
...
bool aop_alu_move_result(asmop *result, asmop *value, bool needs_restore, iCode *ifx) {
    if (AOP_IS_CND(result)) {
        if (needs_restore) {
            ...
            aop_cmp(EQ_OP, ASMOP_ZERO, value, NULL, set_true, set_false, false);
            ...
            aop_cmp(EQ_OP, ASMOP_ZERO, value, NULL, IC_TRUE(ifx), IC_FALSE(ifx), false);
            return true;
        }
        return true;
    }
...
#define MOVE_AOP_DEBUG { emit2("", "; implement me (%s:%d)", __FILE__, __LINE__); emit2(";", "op = %s", alu_operations[op]); emit_asmop("left ", left); emit_asmop("right", right); emit_asmop("result", result); }
    ...
    if (op == ALUS_AND || op == ALUS_OR || op == ALUS_XOR) {
        int size = size_left;
        if (size_right < size) {
            size = size_right;
        }
        emit2(";", "begin bitwise ALU operation");
        ...
        emit2(";", "end bitwise ALU operation");
...
static void genOr    (iCode *ic)             { genALUOp(ALUS_OR,  ic, NULL); }
        ...
        if (byteOfVal(OP_VALUE(right), 0) == 1) {
            ...
            if (size_result == 1 && size_left == 2) {
                ...
                if (AOP_IS_REG(result->aop)) {
                    ...
                    aop_move(aop_byte_of(result->aop, 1), left->aop);
                    aop_move(aop_byte_of(result->aop, 0), ASMOP_ZERO);
                } else {
                    emit2("", "; implement me (%s:%d)", __FILE__, __LINE__);
                    emit_asmop("left ", left->aop);
                    emit_asmop("right ", right->aop);
...
int alus_from_ic_op(unsigned int op) {
...
#define D2(x) /*do { x; } while (0)*/
    ...
    if (true_and_false) {
        emit2(";", "has TRUE and FALSE branch");
    }

    bool store_result = !(true_branch || false_branch);
    ...
    if (left->size == right->size
        ...
        if (left->size == 1) {
            ...
            emit2(";", "begin multibyte (%d) equality check", left->size);
            for (int i = 0; i < max_size; ++i) {
                /* aop_alu(ALUS_NOT, left, NULL, ASMOP_ALUC, NULL); */
                D2(emit2(";", "compare byte %d", i));
                ...
                if (i < max_size - 1) {
                    ...
                    if (true_branch) {
                        ...
                        D2(emit2(";", "Jump to the end so we can store the result."));
                        emit_jump_to_label(end_of_comparison, 0);
                    }
                }
            ...
            emit2(";", "end multibyte equality check");
        } else {
...
static void genCmpEQorNE   (iCode *ic, iCode *ifx)       {
    ...
    if (ifx) {
        aop_cmp(ic->op, left->aop, right->aop, result->aop, IC_TRUE(ifx), IC_FALSE(ifx), false);
    } else {
        aop_cmp(ic->op, left->aop, right->aop, result->aop, NULL, NULL, false);
    }

    if (ifx && left->aop->size == right->aop->size && left->aop->size == 1) {
        /* emit2(";", "genCmpEQorNE calls genIfx"); */
        // hack; should call this from aop_cmp
        genIfx(ifx);
    } else {
        /* emit2(";", "genCmpEQorNE does not call genIfx"); */
    }
}
...
static void genCmp   (iCode *ic, iCode *ifx)       {
    ...
    if (ifx) {
        aop_cmp(ic->op, left->aop, right->aop, NULL, IC_TRUE(ifx), IC_FALSE(ifx), false);
    } else {
        aop_cmp(ic->op, left->aop, right->aop, result->aop, NULL, NULL, false);
    }
    ...
    if (ifx && left->aop->size == right->aop->size && left->aop->size == 1) {
        /* emit2(";", "genCmp calls genIfx"); */
        // hack; should call this from aop_cmp
        genIfx(ifx);
    } else {
        /* emit2(";", "genCmp does not call genIfx"); */
    }
    /* if (ifx) { */
    /*     genIfx(ifx); */
    /* } */
