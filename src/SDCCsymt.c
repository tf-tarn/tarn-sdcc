      ...
      for (muldivmod = 0; muldivmod < 3; muldivmod++)
        {
          ...
          muldiv[muldivmod][bwd][su] =
            funcOfType (_mangleFunctionName (dbuf_c_str (&dbuf)), multypes[(TARGET_IS_PIC16 && muldivmod == 1 && bwd == 0 && su == 0 || (TARGET_IS_PIC14 || TARGET_IS_STM8 || TARGET_Z80_LIKE || TARGET_PDK_LIKE || TARGET_MOS6502_LIKE  || TARGET_IS_TARN ) && bwd == 0) ? 1 : bwd][su % 2], multypes[bwd][su / 2], 2,
